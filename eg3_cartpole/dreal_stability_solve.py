...
if __name__ == '__main__':
    ...
    if not os.path.exists(results_dir):
        results_dir = "{}/eg3_results_keep/{:03d}".format(str(Path(__file__).parent.parent), exp_num)
    ...
    mass_pole = system.mass_pole.item()
    mass_cart = system.mass_cart.item()
    length = system.length.item()
    friction = system.friction_coef.item()
    gravity = system.gravity.item()
    assert friction == 0.0
    x1_dot = x3
    x2_dot = x4
    tmp = mass_cart + mass_pole*sin(x2)*sin(x2)
    x3_dot = (mass_pole*gravity*sin(x2)*cos(x2) - mass_pole*length*x4*x4*sin(x2) + control)/(tmp)
    x4_dot = ((mass_cart+mass_pole)*gravity*sin(x2) - mass_pole*length*x4*x4*sin(x2)*cos(x2) + control*cos(x2))/(length*tmp)

    state_derivative = np.array([x1_dot, x2_dot, x3_dot, x4_dot])  


    grad_V = np.array([V.Differentiate(x1), V.Differentiate(x2), V.Differentiate(x3), V.Differentiate(x4)])
    ...
    norm_x = sqrt(x1*x1 + x2*x2 + x3*x3 + x4*x4)
    ...
    for i in range(N):
        env = {x1: test_input[i, 0].item(), x2: test_input[i, 1].item(), x3: test_input[i, 2].item(), x4: test_input[i, 3].item()}
        t1 = time.time()
    ...
    bound = logical_and(sqrt(x1*x1 + x2*x2 + x3*x3 + x4*x4)>= stability_cutoff_radius,
