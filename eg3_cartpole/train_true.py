...
if __name__ == '__main__':
    ...
    nominal_results_dir = "{}/eg3_results/{:03d}".format(str(Path(__file__).parent.parent), nominal_results_exp_num)
    ...
    actuation_in_features = actuation_config["in_features"]
    actuation_out_features = actuation_config["out_features"]
    actuation_activations = [actuation_config["activations"]]*(actuation_config["num_layers"]-1)
    actuation_widths = [actuation_in_features]+[actuation_config["width_each_layer"]]*(actuation_config["num_layers"]-1)+[actuation_out_features]
    actuation_zero_at_zero = bool(actuation_config["zero_at_zero"])
    actuation_bias = bool(actuation_config["bias"])
    actuation_input_bias = np.array(actuation_config["input_bias"], dtype=config.np_dtype)
    actuation_input_transform = 1.0/np.array(actuation_config["input_transform_to_inverse"], dtype=config.np_dtype)
    actuation_nn = FullyConnectedNetwork(in_features=actuation_in_features, 
                                    out_features=actuation_out_features,
                                    activations=actuation_activations,
                                    widths=actuation_widths,
                                    zero_at_zero=actuation_zero_at_zero,
                                    bias=actuation_bias,
                                    input_bias=actuation_input_bias,
                                    input_transform=actuation_input_transform,
                                    lower_bound=None,
                                    upper_bound=None,
                                    dtype=config.pt_dtype)
    summary(actuation_nn, input_size=(1, actuation_in_features), dtypes=[config.pt_dtype])
    ...
    def estimated_drift(x):
        ...
        learned_part = drift_nn(x) # (batch_size, 2)
        known_part[:, 2:4] += learned_part
    ...
    def estimated_actuation(x):
        ...
        learned_part = actuation_nn(x) # (batch_size, 2)
        known_part[:, 2:4, :] += learned_part.unsqueeze(2) # (batch_size, state_dim, control_dim)
        return known_part
