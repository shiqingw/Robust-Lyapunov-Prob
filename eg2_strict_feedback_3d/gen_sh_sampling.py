...
def generate_sh_script_sampling_solve_on_nom(filename, exp_nums, device, delta, epsilon):
    with open(filename, "a") as file:
        for exp_num in exp_nums:
            ...
            command2 = f"python -u eg2_strict_feedback_3d/sampling_on_nom.py --exp_num {exp_num} " + \
                f"--device {device} " + \
                f"--delta {delta:.1E} " + \
                f"--epsilon {epsilon:.1E} " + \
                f"> eg2_results/{exp_num:03d}/output_sampling_on_nom_{device}_delta_{delta:.1E}_epsilon_{epsilon:.1E}.out\n"
            file.write(command1)
            file.write(command2)
...
def generate_sh_script_sampling_solve_on_true(filename, exp_nums, device, delta, epsilon):
    with open(filename, "a") as file:
        for exp_num in exp_nums:
            command1 = f"mkdir eg2_results/{exp_num:03d}\n"
            command2 = f"python -u eg2_strict_feedback_3d/sampling_on_true.py --exp_num {exp_num} " + \
                f"--device {device} " + \
                f"--delta {delta:.1E} " + \
                f"--epsilon {epsilon:.1E} " + \
                f"> eg2_results/{exp_num:03d}/output_sampling_on_true_{device}_delta_{delta:.1E}_epsilon_{epsilon:.1E}.out\n"
            file.write(command1)
            file.write(command2)
