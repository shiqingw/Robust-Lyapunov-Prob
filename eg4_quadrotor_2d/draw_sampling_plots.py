...
def draw_on_nominal_system(exp_num):
    ...
    results_dir = "{}/eg4_results/{:03d}".format(str(Path(__file__).parent.parent), 0)
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)

    # Exp directory
    print("==> Creating result directory ...")
    exp_dir = "{}/eg4_results/{:03d}".format(str(Path(__file__).parent.parent), exp_num)
    if not os.path.exists(exp_dir):
        exp_dir = "{}/eg4_results/{:03d}_keep".format(str(Path(__file__).parent.parent), exp_num)
...
def draw_on_true_system(exp_num):
    print("==> Exp_num:", exp_num)
    # Create result directory
    results_dir = "{}/eg4_results/{:03d}".format(str(Path(__file__).parent.parent), 0)
    if not os.path.exists(results_dir):
    ...
    for (x_state_idx, y_state_idx) in pairwise_idx:
        ...
        plt.tight_layout()
        plt.savefig(f"{results_dir}/sample_on_true_{state_names[x_state_idx]}_{state_names[y_state_idx]}_{exp_num:03d}.png", dpi=200)
        plt.close()
