...
if __name__ == '__main__':
    ...
    nominal_results_dir = "{}/eg4_results/{:03d}".format(str(Path(__file__).parent.parent), nominal_results_exp_num)
    ...
    def estimated_drift(x):
        ...
        learned_part = drift_nn(x) # (batch_size, 3)
        known_part[:, 3:6] += 0*learned_part
    ...
    def estimated_actuation(x):
        ...
        learned_part = actuation_nn(x) # (batch_size, 3)
        known_part[:, 3:6, 0] += learned_part
        known_part[:, 3:5, 1] += learned_part[:, 0:2]
        known_part[:, 5, 1] -= learned_part[:, 2]
        return known_part
