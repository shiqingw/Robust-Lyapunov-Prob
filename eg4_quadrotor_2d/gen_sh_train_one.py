...
def generate_sh_script_nominal(filename, exp_nums, device):
    with open(filename, "w") as file:
        for exp_num in exp_nums:
            ...
            command2 = f"python -u eg4_quadrotor_2d/train_nominal.py --exp_num {exp_num} --device {device} > eg4_results/{exp_num:03}/output.out\n"
            file.write(command1)
            file.write(command1)
            file.write(command2)
...
def generate_sh_script_true(filename, exp_nums, device):
    with open(filename, "a") as file:
        for exp_num in exp_nums:
            command1 = f"mkdir eg4_results/{exp_num:03}\n"
            command2 = f"python -u eg4_quadrotor_2d/train_true.py --exp_num {exp_num} --device {device} > eg4_results/{exp_num:03}/output.out\n"
            file.write(command1)
            file.write(command1)
            file.write(command2)
