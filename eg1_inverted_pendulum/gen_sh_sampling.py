    ...
    with open(filename, "w") as file:
        for exp_num in exp_nums:
            ...
            command2 = f"python -u eg1_inverted_pendulum/sampling_on_nom.py --exp_num {exp_num} " + \
                ...
                f"> eg1_results/{exp_num:03d}/output_sampling_on_nom_delta_{delta:.1E}_epsilon_{epsilon:.1E}.out\n"
            file.write(command1)
            file.write(command2)
    ...
    with open(filename, "a") as file:
        for exp_num in exp_nums:
            ...
            command2 = f"python -u eg1_inverted_pendulum/sampling_on_true.py --exp_num {exp_num} " + \
                f"--delta {delta:.1E} " + \
                f"--epsilon {epsilon:.1E} " + \
                f"> eg1_results/{exp_num:03d}/output_sampling_on_true_delta_{delta:.1E}_epsilon_{epsilon:.1E}.out\n"
            file.write(command1)
            file.write(command2)
