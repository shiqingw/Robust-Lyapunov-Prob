...
def draw_unperturbed(exp_num):
    ...
    for i in range(num_trajs):
        ax.plot(trajectories_np[i, :, 0], trajectories_np[i, :, 1])
    ...
    plt.tight_layout()
    plt.savefig(f"{results_dir}/unperturbed_{exp_num:03d}_traj.pdf", dpi=100)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{results_dir}/unperturbed_{exp_num:03d}_state_norm.pdf", dpi=100)
    plt.close()
    ...
    ax.hlines(y=0, xmin=0, xmax=horizon*dt, color='black', linestyle='--')
    plt.tight_layout()
    plt.savefig(f"{results_dir}/unperturbed_{exp_num:03d}_control.pdf", dpi=100)
    plt.close()
...
def draw_perturbed(exp_num):
    ...
    ax.tick_params(axis='both', which='major', labelsize=ticksize, grid_linewidth=10)
    plt.tight_layout()
    plt.savefig(f"{results_dir}/perturbed_{exp_num:03d}_traj.pdf", dpi=100)
    plt.close()
    ...
    ax.hlines(y=0, xmin=0, xmax=horizon*dt, color='black', linestyle='--')
    plt.tight_layout()
    plt.savefig(f"{results_dir}/perturbed_{exp_num:03d}_state_norm.pdf", dpi=100)
    plt.close()
    ...
    ax.hlines(y=0, xmin=0, xmax=horizon*dt, color='black', linestyle='--')
    plt.tight_layout()
    plt.savefig(f"{results_dir}/perturbed_{exp_num:03d}_control.pdf", dpi=100)
    plt.close()
