...
if __name__ == '__main__':
    ...
    def estimated_drift(x):
        return drift_nn(x) + nominal_system.get_drift(x) # (batch_size, state_dim)
    ...
    for (x_idx, y_idx) in pairwise_idx:
        diff_mean, diff_max = draw_model_diff_l2_contour(model1=nominal_system.get_drift,
    ...
    state_names = ["theta", "dtheta"]
    for (x_idx, y_idx) in pairwise_idx:
        diff_mean, diff_max = draw_model_diff_l2_contour(model1=nominal_system.get_actuation,
